...
class IveFunction(_torch.autograd.Function):
    @staticmethod
    def forward(self, v, z):
        ...
        return _torch.Tensor(output).to(z.device)
...
def ive_fraction_approx(v, z):
    ...
    return z / (v - 1 + _torch.pow(torch.pow(v + 1, 2) + _torch.pow(z, 2), 0.5))
...
def ive_fraction_approx2(v, z, eps=1e-20):
    def delta_a(a):
        lamb = v + (a - 1.0) / 2.0
        return (v - 0.5) + lamb / (
            2 * _torch.sqrt((_torch.pow(lamb, 2) + _torch.pow(z, 2)).clamp(eps))
        )

    delta_0 = delta_a(0.0)
    delta_2 = delta_a(2.0)
    B_0 = z / (
        delta_0 + _torch.sqrt((_torch.pow(delta_0, 2) + _torch.pow(z, 2))).clamp(eps)
    )
    B_2 = z / (
        delta_2 + _torch.sqrt((_torch.pow(delta_2, 2) + _torch.pow(z, 2))).clamp(eps)
    )
